...
public class OclockView extends View {
    public OclockView(Context context, int i) {
        super(context);
        StartTime = i ;
        init(context);
        StartTime =((Examination)context).GetTime();
    ...
    public OclockView(Context context, @Nullable AttributeSet attrs) {
        super(context, attrs);
        StartTime =((Examination)context).GetTime();
        handler = ((Examination)context).GetHandler();
        init(context);
    }
    ...
    private void init(Context context) {
        ...
        weithColor = Color.parseColor("#237EAD");
        grayColor = Color.parseColor("#88237EAD");
        ...
        sPaint = new Paint();
        sPaint.setColor(Color.parseColor("#237EAD"));
        ...
        mPaint = new Paint();
        mPaint.setColor(Color.parseColor("#237EAD"));
        mPaint.setStyle(Paint.Style.FILL);
        mPaint.setAntiAlias(true);

        hPaint = new Paint();
        hPaint.setColor(Color.parseColor("#88237EAD"));
        hPaint.setStyle(Paint.Style.FILL);
        hPaint.setAntiAlias(true);
        LogUtils.d(String.valueOf(StartTime));
        StartTime(StartTime);
    }
    ...
    private void StartTime(long time) {
        ...
        timer = new CountDownTimer(time, 1) {
            ...
            public void onTick(long millisUntilFinished) {
                double milliSecond = millisUntilFinished;
                double hour = milliSecond/3600000.00;
                milliSecond = milliSecond%3600000.00;
                double minute = milliSecond/60000.00;
                milliSecond = milliSecond%60000.00;
                double second = milliSecond / 1000.0;
                milliSecond = milliSecond%1000.00;

//                float mmilliSecond = millisUntilFinished;
//                //秒
//                float msecond = mmilliSecond / 1000;
//                //分
//                float mminute = msecond / 60;
//                //时
//                float mhour = mminute / 60;
//                float second = msecond - (int)mhour*60 - (int)mminute*60;
//                float minute = mminute - (int)mhour*60;
//                float hour = mhour;

                //LogUtils.d("second = " + (int)second + "  min = " + (int)minute + "  hour = " + (int)hour);

                mSecondDegree = (float) (second * 6.0);
                mMinuteDegree = (float) (6.0 * minute);
                mHourDegree = (float) (30.0 * hour);
            }
    ...
    private void getTime() {
        ...
        LogUtils.d("StartTime"+String.valueOf(hour)+":"+minute+":"+second);
        //求出三个指针的角度 按照360来计算
